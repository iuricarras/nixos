{pkgs, ...}: let
  rofi_bluetooth = pkgs.writeShellScriptBin "rofi_bluetooth" ''
    # Run
    # Import Current Theme
    RASI="$HOME/.config/rofi/bluetooth.rasi"
    ...
    print_status() {
        if power_on; then
            ...
            # Check if an outdated version of ${pkgs.bluez}/bin/bluetoothctl is used to preserve backwards compatibility
            if (( $(echo "$(${pkgs.bluez}/bin/bluetoothctl version | cut -d ' ' -f 2) < 5.65" | bc -l) )); then
    ...
    rofi_command="${pkgs.rofi}/bin/rofi -theme ''${RASI} -dmenu $* -p"
