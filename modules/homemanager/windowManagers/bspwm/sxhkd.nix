{
  ...
  options.sxhkd = {
    enable = lib.mkEnableOption "enables sxhkd keybindings";
  ...
  config = lib.mkIf config.sxhkd.enable {
    ...
    services.sxhkd.keybindings = lib.mkIf config.sxhkd.config {
      # Terminal (alacritty)
      ...
      # Rofi Menus/Applets
      "super + {b,m,n,r,s,t,w,x}" = "{rofi_bluetooth,rofi_music,nmd,rofi_asroot,rofi_screenshot,rofi_themes,rofi_windows,rofi_powermenu}";

      ##---------- Applications ----------##

      # Launch Apps
      "super + shift + {f,w,e}" = "{thunar,firefox,geany}";

      # Terminal Apps
      "ctrl + alt + {v,r,h}" = "alacritty -e {vim,ranger,htop}";
      ...
      "super + shift + space" = "bspc node -t \"~\"{pseudo_tiled,tiled}";
